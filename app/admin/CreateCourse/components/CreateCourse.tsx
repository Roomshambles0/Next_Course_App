...
const router = useRouter();
    ...
    <CreateCard onClick={(title,description,price,image,publish) => {
 axios.post(`/api/admin/courses`, {
      title:title,
       description:description,
       price:price,
       imageLink:image,
       published:publish
    }).then( ()=>{alert("course named " + title +" is created")
    router.push("/admin")});
...
const CreateCard = (props:{
  ...
  const [publish,setPusblish] = useState(true);
  console.log(image)
    ...
    setImage(e.target.value);
      ...
      if(e.target.value=="true"){
        ...
        <div className="hidden peer-checked/draft:block text-md font-medium text-slate-100">Course is only visible to instructer.</div>
